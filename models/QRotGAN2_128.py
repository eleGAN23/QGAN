# -*- coding: utf-8 -*-
"""
Created on Mon Jan  4 11:35:32 2021

@author: Edoardo
"""

''' SSGAN with Linear layers replaced with QuaternionLinear layers'''
...
from utils.Qops import conv2d, Residual_G, Residual_D, First_Residual_D, SNLinear, QSNLinear
from utils.QBN_Vecchi2 import QuaternionBatchNorm2d as QBatchNorm


class QRotGAN2_D128(nn.Module):
    def __init__(self, ssup, spectral_normed=False, channel=4):
        super(QRotGAN2_D128, self).__init__()
        ...
        self.fully_connect_gan2 = QSNLinear(1024, 4, spectral_normed = spectral_normed)
        
        # self.fully_connect_rot2 = nn.Linear(1024, 4)
        # torch.nn.init.normal_(self.fully_connect_rot2.weight.data, std=0.02)
        self.fully_connect_rot2 = QSNLinear(1024, 4, spectral_normed = spectral_normed)
    ...
    def forward(self, x):
        ...
        gan_logits = torch.mean(gan_logits, dim=1, keepdim=True)
        if self.ssup:
...
class QRotGAN2_G128(nn.Module):
    def __init__(self, z_size=128, channel=4, batch_normed=False):
        super(QRotGAN2_G128, self).__init__()

        self.z_size = z_size
        # self.spectral_normed = spectral_normed
        self.batch_normed = batch_normed
        
        self.fully_connect = QSNLinear(z_size, 4*4*1024,  spectral_normed=False)
