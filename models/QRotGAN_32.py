...
from utils.Qops import conv2d, Residual_G, Residual_D, First_Residual_D, SNLinear
...
class QRotGAN_D32(nn.Module):
    def __init__(self, ssup, channel=4, spectral_normed=False):
        super(QRotGAN_D32, self).__init__()
...
class QRotGAN_G32(nn.Module):
    def __init__(self, z_size=128, channel=4, output_size = 32,
                 batch_normed=False):
        super(QRotGAN_G32, self).__init__()
        s = 4
        self.s = s
        self.z_size = z_size
    ...
    def forward(self, x, keep_4_channel):
        ...
        if keep_4_channel:
            d5 = d5.view(d5.size(0), d5.size(1), d5.size(2), d5.size(3), 1)
